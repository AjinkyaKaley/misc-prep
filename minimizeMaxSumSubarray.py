...
class Solution:
    ...
    def minimizeMaxSum(self, A, idx, result, m,startIdx):
        if m == 1:
            result += [A[idx:]]
            print(result)
            del result[-1]
        ...
        for i in range(idx, len(A)):
            if i == startIdx:
                continue
            iterResult = A[startIdx:i]
            result.append(iterResult)
            self.minimizeMaxSum(A, i, result, m - 1, startIdx + i)
            del result[-1]
    ...
    def helper(self, i, nums, m, cache):
        if i == len(nums):
            ...
            for j in range(1, len(nums)+1):
                left, right = sum(
                    nums[i:i+j]), self.helper(i+j, nums, m-1, cache)
